-- Four-state 3-hop MOESI protocol
...
const
	...
	VC0: 0;                -- low priority GetM, GetS, PutE, PutO, PutM, PutS, SelfDowngradeData
	VC1: 1;		       -- Put_Ack, Fwd_GetS, Fwd_GetM
	VC2: 2;		       -- Data
	VC3: 3;		       -- Inv_Ack, Invalidation
	QMax: 2;
	NumVCs: VC3 - VC0 + 1;
...
Message:
	...
	Record
		state: enum { H_Invalid, H_Modified, H_Shared, H_Exclusive, H_Owned,	--stable states
      		    H_SM_A, H_EM_A, H_MM_A, H_OI_A, H_OM_A, H_MO_A, H_OO_A }; 		--transient states during recall
		owner: Node;	
	...
	Record
		state: enum { P_Invalid, P_Shared, P_Modified, P_Exclusive, P_Owned,			--stable states
                  		IS_D, IM_AD, IM_A, SM_AD, SM_A, MI_A,EI_A, SI_A, OI_A_WaitForFwdAck, 		--transient states
		  		II_A, OI_A_WaitForPutAck, OI_A, OM_A, OM_A_WaitForGetMAck,  
		  		OO_A, OI_A_WaitForGetMAck, OS_A, MS_A, MI_A_WaitForSDA			
                  };
		val: Value;
		InvAckNum: InvAckCnt;
		FwdAckNum: FwdAckCnt;
...
Begin
	for n:Node do
		...
		then
			if n != rqst
			then 
				Send(Invalidation, n, rqst, VC3, UNDEFINED,0);
...
--  put " at home -- "; put HomeNode.state;
  ...
  case H_Owned:
    switch msg.mtype
      case GetS:
	    Send(Fwd_GetS, HomeNode.owner, msg.src, VC1, UNDEFINED, cnt);
	    AddToSharersList(msg.src);
	    HomeNode.state:=H_OO_A;
      case GetM:
	if (msg.src=HomeNode.owner)
	then
	  Send(GetM_Ack, msg.src, HomeType, VC1, UNDEFINED, cnt);
	  if (cnt =0)
	  then
	    ...
	    Send(Fwd_GetM, HomeNode.owner, msg.src, VC1, UNDEFINED, cnt);
	    SendInvReqToSharers(msg.src);
	    HomeNode.owner:=msg.src;
	    HomeNode.state:=H_OM_A;
	endif;
...
--  put " at proc "; put p; put "\n";
  ...
  case P_Shared:
    switch msg.mtype	 
	  case Invalidation:
		 ...
		 undefine pv;
		 Send(Inv_Ack, msg.src, p, VC3, UNDEFINED, 0);
		 Send(Inv_Ack, HomeType, p, VC3, UNDEFINED, 0);
	  case Fwd_GetS:
		Send(OwnedData, msg.src, p, VC2, pv, 0);
  ...
  case P_Modified: 
    switch msg.mtype
      case Fwd_GetS:
        Send(FwdData, msg.src, p, VC2, pv, 0);
	fan:=fan+1;
        ps := OO_A;
        
      case Fwd_GetM:
      	Send(Data, msg.src, p, VC2, pv, msg.sharenum);
        Send(Fwd_Ack, HomeType, p, VC2, UNDEFINED, 0);
      	ps := P_Invalid;
      	undefine pv;
  ...
  case P_Owned:
	...
	if (fan<ProcCount)
	then
        Send(FwdData, msg.src, p, VC2, pv, msg.sharenum);
	fan:=fan+1;
  ...
  case IS_D:
	  ...
	  case FwdData:
		...
		pv := msg.val;
		Send(Fwd_Ack, HomeType, p, VC2, UNDEFINED, 0);
	        Send(Fwd_Ack, msg.src, p, VC2, UNDEFINED, 0);
	case OwnedData:
	    pv :=msg.val;
	    ps :=P_Shared;
	    Send(Fwd_Ack, HomeType, p, VC2, UNDEFINED, 0);
  ...
  case OM_A_WaitForGetMAck:
	...
	Send(OwnedData, msg.src, p, VC2, pv, 0);
      ...
      case Fwd_GetM:
	  Send(OwnedData, msg.src, p, VC2, pv, msg.sharenum);
	  ps := IM_AD;
...
ruleset n:Proc Do
  alias p:Procs[n] Do
	...
	rule "self downgrade"
	  ...
	  then
	    p.state := MS_A;
	    Send(SelfDowngradeData, HomeType, n, VC0, p.val, 0)
	  endif;
	 if (p.state = P_Owned)
	  then
	    p.state:= OS_A;
    ...
    rule "replacement(writeback)"
      ...
      if (p.state = P_Owned)
      then
        Send(PutO, HomeType, n, VC0, p.val, 0);
        p.state := OI_A;
      endif;
    endrule;
