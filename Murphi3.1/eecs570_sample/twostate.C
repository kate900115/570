/******************************
  Program "twostate.m" compiled by "Murphi Release 3.1"
...
#define PROTOCOL_NAME "twostate"
#define BITS_IN_WORLD 298
...
class mu_1_Node: public mu__byte
{
 public:
  ...
  inline int value(int val)
  {
    ...
    if ((val >= 6) && (val <= 6)) return (mu__byte::value(val-6)+6);
    if ((val >= 1) && (val <= 3)) return (mu__byte::value(val-0)+0);
  }
...
void mu_1_Message::set_self(char *n, int os)
{
  ...
  mu_src.set_self_2(name, ".src", os + 3 );
  mu_vc.set_self_2(name, ".vc", os + 6 );
...
void mu_1_HomeState::set_self(char *n, int os)
{
  ...
  mu_owner.set_self_2(name, ".owner", os + 2 );
  mu_val.set_self_2(name, ".val", os + 5 );
}
...
void mu_1_ProcState::set_self(char *n, int os)
{
  name = n;
  mu_state.set_self_2(name, ".state", os + 0 );
  mu_val.set_self_2(name, ".val", os + 3 );
}
...
void mu_1__type_2::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"Proc_1", i * 5 + os);i++;
array[i].set_self_ar(n,"Proc_2", i * 5 + os);i++;
array[i].set_self_ar(n,"Proc_3", i * 5 + os);i++;
}
...
void mu_1__type_3::set_self( char *n, int os)
{
  ...
  name = n;
  for(i = 0; i < 4; i++)
    array[i].set_self(tsprintf("%s{%d}", n,i), i * 10 + os);
  k = os + i * 10;
  for(i = 0; i < 4; i++)
...
void mu_1__type_4::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"HomeType", i * 48 + os);i++;
...
void mu_1__type_6::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"HomeType", i * 20 + os);i++;
array[i].set_self_ar(n,"Proc_1", i * 20 + os);i++;
array[i].set_self_ar(n,"Proc_2", i * 20 + os);i++;
array[i].set_self_ar(n,"Proc_3", i * 20 + os);i++;
}
...
/*** Variable declaration ***/
mu_1__type_2 mu_Procs("Procs",7);

/*** Variable declaration ***/
mu_1__type_4 mu_Net("Net",22);

/*** Variable declaration ***/
mu_1__type_6 mu_InBox("InBox",214);

/*** Variable declaration ***/
mu_0_boolean mu_msg_processed("msg_processed",294);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",296);
