/******************************
  Program "pingpong.m" compiled by "Murphi Release 3.1"

  Murphi Last Modefied Date: "Jan 29 1999"
  Murphi Last Compiled date: "Mar  3 2016"
 ******************************/
...
void mu_1__type_0::set_self(char *n, int os)
{
  ...
  mu_gotball.set_self_2(name, ".gotball", os + 2 );
}
...
/********************
  Invariant records
 ********************/
int mu__invariant_2() // Invariant "Each player either hasball or gotball."
{
bool mu__quant3; 
mu__quant3 = TRUE;
{
...
bool mu__boolexpr4;
bool mu__boolexpr5;
  if (!((mu_Players[mu_p].mu_hasball) == (mu_true))) mu__boolexpr5 = TRUE ;
  else {
  mu__boolexpr5 = ((mu_Players[mu_p].mu_gotball) == (mu_false)) ; 
}
  if (!(mu__boolexpr5)) mu__boolexpr4 = FALSE ;
  else {
bool mu__boolexpr6;
  if (!((mu_Players[mu_p].mu_gotball) == (mu_true))) mu__boolexpr6 = TRUE ;
  else {
  mu__boolexpr6 = ((mu_Players[mu_p].mu_hasball) == (mu_false)) ; 
}
  mu__boolexpr4 = (mu__boolexpr6) ; 
}
if ( !(mu__boolexpr4) )
...
bool mu__condition_7() // Condition for Rule "Each player either hasball or gotball."
{
...
int mu__invariant_8() // Invariant "Only one ball in play."
{
bool mu__quant9; 
mu__quant9 = TRUE;
{
...
bool mu__boolexpr11;
bool mu__boolexpr12;
  if (!(mu_Players[mu_p].mu_hasball)) mu__boolexpr12 = FALSE ;
  else {
  mu__boolexpr12 = (mu_Players[mu_p].mu_gotball) ; 
}
  if (!(!(mu__boolexpr12))) mu__boolexpr11 = FALSE ;
  else {
bool mu__boolexpr13;
  if (mu_Players[mu_p].mu_hasball) mu__boolexpr13 = TRUE ;
  else {
  mu__boolexpr13 = (mu_Players[mu_p].mu_gotball) ; 
}
...
bool mu__boolexpr15;
bool mu__boolexpr16;
  if (mu_Players[mu_q].mu_hasball) mu__boolexpr16 = TRUE ;
  else {
  mu__boolexpr16 = (mu_Players[mu_q].mu_gotball) ; 
}
  if (!(mu__boolexpr16)) mu__boolexpr15 = TRUE ;
  else {
  mu__boolexpr15 = ((mu_p) == (mu_q)) ; 
}
if ( !(mu__boolexpr15) )
...
bool mu__condition_17() // Condition for Rule "Only one ball in play."
{
...
{"Only one ball in play.", &mu__condition_17, NULL, FALSE},
{"Each player either hasball or gotball.", &mu__condition_7, NULL, FALSE},
};
